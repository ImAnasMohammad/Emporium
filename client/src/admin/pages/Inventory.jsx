...
const Inventory = () => {
    ...
    const getAllProducts = async (url = `${serverURL}/products/${currentPage}/${sort}`) => {
    ...
    useEffect(() => {
        if (search === '') {
            ...
            console.log('search')
            getAllProducts(`${serverURL}/products/search/${search}/${currentPage}`);
        }
    }, [sort, currentPage]);
        ...
        <>
            <Layout page='Products' isLoading={isLoading}>
                ...
                <Table headings={tableHeadings}>
                    {products.length > 0 && !isLoading && <AllRows data={products} handleDeleteProduct={handleDeleteProduct} page={currentPage-1} limit={10} />}
                </Table>
...
const AllRows = ({ data, handleDeleteProduct,page,limit }) => {
    return <>
        {
            data?.map((data, index) => <SingleRow data={data} key={index} sno={(page*limit)+index+1} handleDeleteProduct={handleDeleteProduct} />)
        }
    </>
}
